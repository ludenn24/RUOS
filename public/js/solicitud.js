...
$("#periodo").val(output);
    ...
    function formato(d) {
        return '<div class="col-md-4"><strong>Número de resolución: </strong> ' + d.res_dis + '<br>' +
            '<strong>Inicio de vigencia: </strong> ' + d.inicio + '<br>' +
            '<strong>Fin de Vigencia: </strong>' + d.fin + '</div>';
    ...
    function ListarSolicitudes(cod_org) {
...

                {
                    ...
                    render: function (data, type, row) {      
                        return "<a class=''" +
                            ...
                            "data-target='#JuntaDirectiva'" +
                            "data-toggle='modal'" +
                            "data-codigo='" + row.codigo + "'>" +
                            "(" + row.miembros + ") Miembros</a>"
                    },
                },
                
                  {
                    ...
                    render: function (data, type, row) {      
                        return "<a class=''" +
                            ...
                            "data-toggle='modal'" +
                            "data-codigo='" + row.codigo + 
                            "data-cod_org='" + row.cod_org + "'>" +
                            "(" + row.documentos + ") Subir</a>"
                    },
                },
        ...
        $('#ListaSolicitudes tbody').off('click', 'tr td.details-control');
        $('#ListaSolicitudes tbody').on('click', 'tr td.details-control', function () {
    ...
    !function (a) {
        ...
        a(function () {
            ...
            a("#RegistrarMiembro").on("click", function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "../juntadirectiva/registrarrepresentante",
                    data: dataString,
                    dataType: 'json',
                    beforeSend: function () {
                        b.button("loading");
...
function toggleBounce() {
    ...
    function ListarMiebros(cod_sol) {

        responsive: true

        document.getElementById("cod_sol").value = cod_sol;
        $codigo_organizacion = cod_sol;
        ...
        var dt = $('#MiembrosJuntaDirectiva').DataTable({
            ...
            "ajax": {
                "type": "get",
                "url": "../juntadirectiva/listarepresentantes",
                "data": {cod_sol: cod_sol,}
            },
