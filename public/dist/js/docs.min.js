!function (a) {
    ...
    a(function () {
        ...
        a("#updateorg").on("click", function () {
          ...
          a("#updatesolicitante").on("click", function () {
            var b = a(this), dataString;
            var holder = '';
            var exito = true;
            $(".obligate-sol").each(function () {
              if ($(this).val() == '') {
                $(this).focus();
                holder = $(this).attr("data-content");
                exito = false;
                $(".msgsol").html("").show();
                $(".msgsol").html("<div class='error'><i class='fa fa-exclamation-triangle'></i>" + holder + "</div>");
                setTimeout(function () {
                  $(".msgsol").fadeOut(1500);
                }, 1500);
                return false;
            ...
            if (!exito)
              return false;
            dataString = $('#FormActualizarSolicitante').serialize();
            $.ajax({
              type: "POST",
              url: '{{ base_url() }}/../../../admin/actualizarsolicitante',
              data: dataString,
              dataType: 'json',
              beforeSend: function () {
                b.button("Guardando");
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, item) {
                        ...
                        $(".timeline").append("" +
                                "<li>" +
                                "<a download href='../../" + item.urldoc + "'><i class='fa fa-folder'></i> " + item.urldoc + "</a>" +
        ...
        a("#aprobar-solicitud").on("click", function () {
         ...
         a("#eliminar-resolucion").on("click", function () {
            var b = a(this), dataString;
            dataString = $('#eliminar_res').serialize();
            $.ajax({
                type: "POST",
                url: '{{ base_url() }}/../../../eliminarresolcuion',
                data: dataString,
                beforeSend: function () {
                },
                success: function (data) {
                    b.button("reset");
                    $('#EliminarResolucion').modal('hide')
                    $('#modalinfo').find(".modal-body").html("<center>" + data + "</center>");
                    $('#modalinfo').modal('show');
                    setTimeout(function () {
                        location.reload();
        ...
        a("#atenderasesoria").on("click", function () {
            var b = a(this), dataString;
            var holder = '';
            var exito = true;

            $(".obligateasesoria").each(function () {
                if ($(this).val() == '') {
                    $(this).focus();
                    holder = $(this).attr("data-content");
                    exito = false;
            ...
            if (!exito)
                return false;
            dataString = $('#asesoria').serialize();
            $.ajax({
                type: "POST",
                url: "{{ base_url() }}/../../admin/atenderasesoria",
                data: dataString,
                dataType: 'json',
                beforeSend: function () {
                    b.button("loading");
                ...
                success: function (data) {
                    ...
                    if (data.response == 'aprobado') {
                        $(".msg").html("<div class='alert alert-warning'><i class='fa fa-exclamation-triangle'></i> La asesoría ya fue atendida anteriormente.</div>");
                        setTimeout(function () {
                            $(".msg").fadeOut(1500);
                            $('#modalAtencion').modal('hide');
                            b.button("reset");
                            $('#asesoria').trigger("reset");
                        }, 2500);

                    } else if (data.response == 'atendido') {
                        $(".msg").html("<div class='alert alert-success'><i class='fa fa-exclamation-triangle'></i> La asesoría se atendió con éxito.</div>");
                        setTimeout(function () {
                            $(".msg").fadeOut(1500);
                            $('#modalAtencion').modal('hide');
                            b.button("reset");
